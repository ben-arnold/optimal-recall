...
#--------------------------------------
# recall-empirical-plots-cough-RRRD.R
# Ben Arnold
# 20 Oct 2011
#
...
# Anemia
load("~/dropbox/articles/wsp-recall/programs/final/4-cough-analysis/recall-empirical-cough-anemia.RData")
aihp <- ihp
aimp <- imp
aind <- ind
aper <- per
...
# HAZ < -2
load("~/dropbox/articles/wsp-recall/programs/final/4-cough-analysis/recall-empirical-cough-haz.RData")
hihp <- ihp
himp <- imp
hind <- ind
hper <- per
...
# WAZ < -2
load("~/dropbox/articles/wsp-recall/programs/final/4-cough-analysis/recall-empirical-cough-waz.RData")
wihp <- ihp
wimp <- imp
wind <- ind
wper <- per
...
pdf("~/dropbox/articles/wsp-recall/figures/cough/empirical-cough-RRdp.pdf",width=12,height=15)
lo <- layout(mat=matrix(1:20,nrow=5,ncol=4))
...
pdf("~/dropbox/articles/wsp-recall/figures/cough/empirical-cough-RRpp.pdf",width=12,height=15)
lo <- layout(mat=matrix(1:20,nrow=5,ncol=4),width=c(0.4,1,1,1))
op <- par(mar=c(4,2,2,1)+0.1)
ytics <- c(0.5,1,2,4)
...
RRplot(p1=aihp$mpp1,p0=aihp$mpp0,RR=aihp$RRpp,ytics,main="")
...
pdf("~/dropbox/articles/wsp-recall/figures/cough/empirical-cough-RDdp.pdf",width=12,height=15)
lo <- layout(mat=matrix(1:20,nrow=5,ncol=4))
op <- par(mar=c(4,2,2,1)+0.1)
...
RDplot(p1=wimp$mdp1,p0=wimp$mdp0,RD=wimp$RDdp,ytics,main="WAZ < -2")
...
pdf("~/dropbox/articles/wsp-recall/figures/cough/empirical-cough-RDpp.pdf",width=12,height=15)
lo <- layout(mat=matrix(1:20,nrow=5,ncol=4))
op <- par(mar=c(4,2,2,1)+0.1)
ytics <- seq(-0.1,0.2,by=0.05)
...
RDplot(p1=wimp$mpp1,p0=wimp$mpp0,RD=wimp$RDpp,ytics,main="WAZ < -2")
